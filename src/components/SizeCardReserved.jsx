      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 justify-items-center">
          {sizeGuides.map((guide, index) => (
            <div
              key={index}
              className="bg-white p-6 rounded-lg shadow-md min-h-[450px] flex flex-col max-w-xs mx-auto"
            >
              <img
                src={guide.image}
                alt={guide.title}
              ...
              {/* Botón */}
              <button 
                className="px-8 py-3 border-2 border-blue-300 rounded-full font-bold text-blue-600 bg-white hover:bg-blue-50 transition"
              >
                Reservar
              </button>
            </div>
